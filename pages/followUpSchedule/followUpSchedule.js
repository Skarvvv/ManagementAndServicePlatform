// pages/followUpSchedule/followUpSchedule.js
...
Page({
  ...
  getType:function(e){
    ...
    let drealname = e.currentTarget.dataset.drealname
    this.setData({
      did: did,
      drealname: drealname,
      isSelect: false,
  ...
  doctorList: function(){
    const app = getApp()
    wx.request({
      url: app.globalData.requestAddr + '/doctor/doctorList',
      method: 'GET',
      data: {},
      success: (result) => {
        this.setData({
  ...
  updateFcontent: function(e){
    this.setData({
      fcontent: e.detail.value
    })
  },
  submitFollowUpSchedule: function(){
    const app = getApp();
    wx.request({
      url: app.globalData.requestAddr + '/followup/save',
      method: 'GET',
      data: {
        uid: app.globalData.userInfo.uid,
        did: this.data.did,
        fcontent: this.data.fcontent,
        ftime: this.data.time
      },
      success: (result) => {
        wx.showToast({
  ...
  loadFollowUpSchedules: function(){
    const app = getApp();
    wx.request({
      url: app.globalData.requestAddr + '/followup/followupsByUid',
      method: 'GET',
      data: {
        uid: app.globalData.userInfo.uid
      },
